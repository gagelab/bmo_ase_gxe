#!/bin/bash
#BSUB -J star_wasp[1-30]
#BSUB -o logs/star_wasp_%J_%I.out
#BSUB -e logs/star_wasp_%J_%I.err
#BSUB -n 10
#BSUB -R "span[hosts=1]"
#BSUB -M 40000
#BSUB -W 48:00

# Purpose: Perform two-pass STAR mapping with WASP filtering.
# Input: sample_list.txt with one sample name per line.
# Expected reads: 00_trimmedData/<sample>_trimmed_R1.fq.gz and
#                 00_trimmedData/<sample>_trimmed_R2.fq.gz
# Expected VCF: 06_SampleSpecificVCF/<sample>_phasedGT.vcf
# Output: WASP-filtered, read-grouped BAM files in 07_starMappingWasp/pass2/
...
star_output_dir="$PWD/07_starMappingWasp"
...
if [[ ! -s "$sample_vcf" ]]; then
    echo "ERROR: Sample-specific VCF not found or empty: $sample_vcf" >&2
    exit 1
fi
...
aligned_bam="${pass2_prefix}Aligned.sortedByCoord.out.bam"
wasp_pass_bam="${star_pass2_dir}/${sample}_WASPfilterPASS.bam"
final_bam="${star_pass2_dir}/${sample}_WASPfilterPASS_RG_sort.bam"
...
echo "[${sample}] Step 3: Run second-pass STAR alignment with WASP."
...
STAR \
    ...
    --varVCFfile "$sample_vcf" \
    --outSAMattributes NH HI AS nM vA vG vW \
    --outSAMtype BAM SortedByCoordinate

rm -rf "$second_pass_index"

echo "[${sample}] Step 4: Retain reads that passed WASP filtering."

# With the STAR attribute order specified above, the vW tag was verified
# to occur in SAM field 18. A value of vW:i:1 indicates a WASP-passing read.
samtools view -h "$aligned_bam" |
    awk '$1 ~ /^@/ || $18 == "vW:i:1"' |
    samtools view -Shb - \
    > "$wasp_pass_bam"

if [[ ! -s "$wasp_pass_bam" ]]; then
    echo "ERROR: WASP-passing BAM was not created: $wasp_pass_bam" >&2
    exit 1
fi
